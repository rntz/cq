...
cq has a simple rcfile-like configuration mechanism. The B<-c> option to B<cq
create> specifies a configuration file to use with the created instance; this
may also be used to add a config file to an already-existing instance. Config
files are line-oriented: the first word of a line specifies the configuration
variable to set, and the remainder specifies its value. Unix-style comments
(from '#' until end-of-line) are supported, and empty lines are ignored.
Configuration variables and their descriptions follow.
